...
{
    public class XPagination(HttpClient httpClient, ILocalStorageService localStorageService,
    IJSRuntime jS, XNavMenu xnav) : XDtoClient(httpClient, localStorageService, jS)
    {
        private readonly XNavMenu _nav = xnav;
        ...
        private async Task LoadPag()  // Sparade värden i LocalStorage.
        {
            try
            {
                PagOn = bool.Parse(await GetLS($"Pag{PagId}On"));
                PagAuto = bool.Parse(await GetLS($"Pag{PagId}Auto"));
                PagRows = int.Parse(await GetLS($"Pag{PagId}Rows"));
                PagRed = int.Parse(await GetLS($"Pag{PagId}Red"));
                PagCount = 0;
                PagPage = 1;
                ClassPag = "d-flex";
            }
        ...
        {
            var response = await GetStorage<string>(target);
